...
export function IconButton ({icon: Icon, size, type, onClick}: {icon: Icon, size?: number, type?: "regular" | "destructive", onClick: () => void}){
    ...
    return <button onClick={() => onClick()} className={`inset-ring-4 rounded-lg hover:cursor-pointer p-3 font-bold hover:inset-ring-0 hover:bg-linear-to-br ${buttonType}`}>
        <Icon size={size ?? 32} />
    </button>
}